export default class Pathfinder {
    /**
     * @param {MazeGenerator} maze The maze to pathfind
     */
    constructor(maze) {
        this.maze = maze;
        this.grid = [];
    ...
    reset() {
        for (let y = 0; y < this.maze.height; y++) {
            ...
            for (let x = 0; x < this.maze.width; x++) {
                this.grid[y][x] = this.maze.get(x, y);
            }
        }
    }
    ...
    getNeighbors(node) {
        ...
        if (x < this.maze.width - 1 && this.grid[y][x + 1] !== 1) {
            neighbors.push({ x: x + 1, y: y, g: 0, h: 0, parent: null });
        }
        ...
        if (y < this.maze.height - 1 && this.grid[y + 1][x] !== 1) {
            neighbors.push({ x: x, y: y + 1, g: 0, h: 0, parent: null });
        }
